...
class TestSpruceForestRegressor:
    def test_init(self):
        _ = SpruceForestRegressor()

    def test_fit(self, boston_X, boston_y):
        forest = SpruceForestRegressor()
    ...
    def test_verbose(self, boston_X, boston_y, verbose, capfd):
        forest = SpruceForestRegressor(verbose=verbose)
        forest.fit(boston_X, boston_y)
    ...
    def test_importance_pvalues(self, boston_X_mod, boston_y, importance, mod):
        rfc = SpruceForestRegressor(importance=importance)
        np.random.seed(42)
    ...
    def test_mtry(self, boston_X, boston_y, mtry):
        forest = SpruceForestRegressor(mtry=mtry)
    ...
    def test_inbag(self, boston_X, boston_y):
        inbag = [[1, 2, 3], [2, 3, 4]]
        forest = SpruceForestRegressor(n_estimators=2, inbag=inbag)
        forest.fit(boston_X, boston_y)

        # inbag list different length from n_estimators
        forest = SpruceForestRegressor(n_estimators=1, inbag=inbag)
        with pytest.raises(ValueError):
            forest.fit(boston_X, boston_y)

        # can't use inbag with sample weight
        forest = SpruceForestRegressor(inbag=inbag)
        with pytest.raises(ValueError):
            forest.fit(boston_X, boston_y, sample_weight=[1] * len(boston_y))

        # can't use class sampling and inbag
        forest = SpruceForestRegressor(inbag=inbag, sample_fraction=[1, 1])
        with pytest.raises(ValueError):
            forest.fit(boston_X, boston_y)

    def test_sample_fraction(self, boston_X, boston_y):
        forest = SpruceForestRegressor(sample_fraction=0.69)
        forest.fit(boston_X, boston_y)
    ...
    def test_sample_fraction_replace(self, boston_X, boston_y, replace):
        forest = SpruceForestRegressor(replace=replace)
        forest.fit(boston_X, boston_y)
    ...
    def test_split_rule(self, boston_X, boston_y, split_rule):
        forest = SpruceForestRegressor(split_rule=split_rule)
        ...
        if split_rule == "extratrees":
            ...
            forest = SpruceForestRegressor(split_rule=split_rule, num_random_splits=2)
            with pytest.raises(ValueError):
                forest.fit(boston_X, boston_y)
    ...
    def test_split_select_weights(self, boston_X, boston_y):
        ...
        forest = SpruceForestRegressor(n_estimators=n_trees)
    ...
    def test_regularization(self, boston_X, boston_y):
        ...
        for r in [[1.1], [-0.1], [1, 1]]:
            forest = SpruceForestRegressor(regularization_factor=r)
            with pytest.raises(ValueError):
                forest.fit(boston_X, boston_y)

        # vector of ones isn't applied
        forest = SpruceForestRegressor(regularization_factor=[1] * boston_X.shape[1])
        forest.fit(boston_X, boston_y)
        assert forest.regularization_factor_ == []
        assert not forest.use_regularization_factor_
        ...
        reg = [0.5]
        forest = SpruceForestRegressor(regularization_factor=reg, n_jobs=2)
    ...
    def test_always_split_features(self, boston_X, boston_y):
        forest = SpruceForestRegressor(always_split_features=[0])
        forest.fit(boston_X, boston_y)
    ...
    def test_estimators_(self, boston_X, boston_y):
        forest = SpruceForestRegressor(n_estimators=10)
        with pytest.raises(AttributeError):
            _ = forest.estimators_
        forest.fit(boston_X, boston_y)
        with pytest.raises(ValueError):
            _ = forest.estimators_
        forest = SpruceForestRegressor(n_estimators=10, enable_tree_details=True)
        forest.fit(boston_X, boston_y)
        estimators = forest.estimators_
        assert len(estimators) == 10
        assert isinstance(estimators[0], SpruceTreeRegressor)
    ...
    def test_get_estimator(self, boston_X, boston_y):
        ...
        assert isinstance(estimator, SpruceTreeRegressor)
        with pytest.raises(IndexError):
            _ = forest.get_estimator(idx=20)

    def test_check_estimator(self):
        check_estimator(SpruceForestRegressor())
