...
class TestSpruceForestSurvival:
    def test_init(self):
        _ = SpruceForestSurvival()

    def test_fit(self, lung_X, lung_y):
        forest = SpruceForestSurvival(n_estimators=N_ESTIMATORS)
    ...
    def test_verbose(self, lung_X, lung_y, verbose, capfd):
        forest = SpruceForestSurvival(verbose=verbose)
        forest.fit(lung_X, lung_y)
    ...
    def test_importance_pvalues(self, lung_X_mod, lung_y, importance, mod):
        forest = SpruceForestSurvival(importance=importance)
        np.random.seed(42)
    ...
    def test_mtry(self, lung_X, lung_y, mtry):
        forest = SpruceForestSurvival(mtry=mtry)
    ...
    def test_inbag(self, lung_X, lung_y):
        inbag = [[1, 2, 3], [2, 3, 4]]
        forest = SpruceForestSurvival(n_estimators=2, inbag=inbag)
        forest.fit(lung_X, lung_y)

        # inbag list different length from n_estimators
        forest = SpruceForestSurvival(n_estimators=1, inbag=inbag)
        with pytest.raises(ValueError):
            forest.fit(lung_X, lung_y)

        # can't use inbag with sample weight
        forest = SpruceForestSurvival(inbag=inbag)
        with pytest.raises(ValueError):
            forest.fit(lung_X, lung_y, sample_weight=[1] * len(lung_y))

        # can't use class sampling and inbag
        forest = SpruceForestSurvival(inbag=inbag, sample_fraction=[1, 1])
        with pytest.raises(ValueError):
            forest.fit(lung_X, lung_y)

    def test_sample_fraction(self, lung_X, lung_y):
        forest = SpruceForestSurvival(sample_fraction=0.69)
        forest.fit(lung_X, lung_y)
    ...
    def test_sample_fraction_replace(self, lung_X, lung_y, replace):
        forest = SpruceForestSurvival(replace=replace)
        forest.fit(lung_X, lung_y)
    ...
    def test_split_rule(self, lung_X, lung_y, split_rule):
        forest = SpruceForestSurvival(split_rule=split_rule)
        ...
        if split_rule != "extratrees":
            forest = SpruceForestSurvival(split_rule=split_rule, num_random_splits=2)
            with pytest.raises(ValueError):
                forest.fit(lung_X, lung_y)
    ...
    def test_split_select_weights(self, lung_X, lung_y):
        ...
        forest = SpruceForestSurvival(n_estimators=n_trees)
    ...
    def test_regularization(self, lung_X, lung_y):
        ...
        for r in [[1.1], [-0.1], [1, 1]]:
            forest = SpruceForestSurvival(regularization_factor=r)
            with pytest.raises(ValueError):
                forest.fit(lung_X, lung_y)

        # vector of ones isn't applied
        forest = SpruceForestSurvival(regularization_factor=[1] * lung_X.shape[1])
        forest.fit(lung_X, lung_y)
        assert forest.regularization_factor_ == []
        assert not forest.use_regularization_factor_
        ...
        reg = [0.5]
        forest = SpruceForestSurvival(regularization_factor=reg, n_jobs=2)
    ...
    def test_estimators_(self, lung_X, lung_y):
        forest = SpruceForestSurvival(n_estimators=10)
        with pytest.raises(AttributeError):
            _ = forest.estimators_
        forest.fit(lung_X, lung_y)
        with pytest.raises(ValueError):
            _ = forest.estimators_
        forest = SpruceForestSurvival(n_estimators=10, enable_tree_details=True)
        forest.fit(lung_X, lung_y)
        estimators = forest.estimators_
        assert len(estimators) == 10
        assert isinstance(estimators[0], SpruceTreeSurvival)
    ...
    def test_get_estimator(self, lung_X, lung_y):
        ...
        assert isinstance(estimator, SpruceTreeSurvival)
    ...
    #     check_estimator(SpruceForestSurvival())
